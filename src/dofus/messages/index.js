...
const {message_log_type} = require('../../../aivy_config.json');
...
const set_log = log_name => {
    remove_handler('all', show_name_log);
    remove_handler('all', show_content_log);
    remove_handler('all', show_full_log);
    switch(log_name){
        case 'name': add_handler('all', show_name_log);break;
        case 'content': add_handler('all', show_content_log); break;
        case 'full': add_handler('all', show_full_log); break;
        default: break;
    }
}

set_log(message_log_type);
